...
Generates a 500-row synthetic survey dataset that simulates realistic UAE
respondent demographics, financial behaviour, and app feature preferences.

Output
------
data/uae_finapp_dataset.csv
"""
...
# Income stratified by employment (AED / month)
income_map = {
    "Student":       (2_000,  6_000),
...
monthly_income = np.array(
    [np.random.randint(*income_map[e]) for e in employment]
)

# ── Financial Profile ──────────────────────────────────────────────────────
financial_literacy = np.random.choice(
    [1, 2, 3, 4, 5],
    N,
    p=[0.08, 0.17, 0.30, 0.28, 0.17],
)
...
# ── App Feature Usage (binary flags) ──────────────────────────────────────
...
# Sharia filter usage is correlated with Sharia preference
...
# ── Behavioural Metrics ────────────────────────────────────────────────────
app_sessions_pw = np.clip(
    np.random.poisson(5, N) + financial_literacy - 2, 1, 20
).astype(int)
...
# ── Regression Target: Monthly Investment Amount (AED) ────────────────────
...
# ── Classification Target: Will Adopt Micro-Investment? ───────────────────
prob_adopt = np.clip(
    0.15
...
df = pd.DataFrame({
    ...
    "app_sessions_per_week":      app_sessions_pw,
...
out_path = os.path.join("data", "uae_finapp_dataset.csv")
...
print(f"  Adoption  : {df['will_adopt_microinvestment'].mean():.1%}")
